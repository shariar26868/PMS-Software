...
export default function GanttTimelineView() {
  const { filteredFeatures = [], moduleList = [], getFeatureProgress, activeProject, getProjectTimelineInfo, setIsDeadlineModalOpen, overallProgress = 0 } = useProject();
  
  const timelineInfo = (getProjectTimelineInfo && getProjectTimelineInfo(activeProject)) || {
  ...
  const safeModules = Array.isArray(moduleList) ? moduleList : [];
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className={`px-2.5 py-0.5 rounded-lg text-xs font-bold ${
                  ...
                  {timelineInfo.status || 'On Track'}
                </span>
              </div>
              <p className="text-xs text-slate-400 mt-0.5">
                Target completion date and feature roadmap schedule for <strong className="text-indigo-400">{activeProject?.name || 'Active Project'}</strong>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="bg-slate-950 px-4 py-2.5 rounded-2xl border border-slate-800 text-right space-y-0.5 flex-1 md:flex-none">
              ...
              <span className="text-sm font-bold text-slate-100 font-mono">{timelineInfo.targetCompletionDate || 'N/A'}</span>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-slate-800/80">
          <div className="bg-slate-950 p-3.5 rounded-2xl border border-slate-800/80 space-y-1.5">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-bold font-mono text-indigo-300">
                {(timelineInfo.daysRemaining ?? 0) >= 0 ? `${timelineInfo.daysRemaining ?? 0} Days Left` : `${Math.abs(timelineInfo.daysRemaining ?? 0)} Days Overdue`}
              </span>
            </div>
            <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
              <div
                className="bg-indigo-500 h-full rounded-full transition-all duration-500"
                style={{ width: `${Math.min(100, timelineInfo.timeElapsedPercent || 0)}%` }}
              />
            </div>
            <span className="text-[10px] text-slate-500 block text-right font-mono">
              Started: {timelineInfo.startDate || 'N/A'} ({timelineInfo.timeElapsedPercent || 0}% time elapsed)
            </span>
          </div>
          ...
          <div className="bg-slate-950 p-3.5 rounded-2xl border border-slate-800/80 space-y-1.5">
            ...
            <span className="text-[10px] text-slate-500 block text-right font-mono">
              {safeFeatures.filter(f => getFeatureProgress && getFeatureProgress(f) === 100).length} of {safeFeatures.length} features completed
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-6 overflow-x-auto">
        ...
        {safeModules.map((moduleName) => {
          const modFeatures = safeFeatures.filter(f => f && f.module === moduleName);
            ...
            <div key={moduleName} className="space-y-3 pb-4 border-b border-slate-800/80 last:border-0 last:pb-0">
              ...
              <div className="space-y-3">
                {modFeatures.map((feature, idx) => {
                  ...
                  const deadlineStr = String(feature.deadline || '15');
                  const deadlineDay = parseInt(deadlineStr.split('-').pop()) || (idx + 1) * 4;
                  ...
                  return (
                    <div key={feature.id} className="grid grid-cols-12 gap-2 items-center bg-slate-950 p-3 rounded-2xl border border-slate-800/80 hover:border-indigo-500/40 transition-all">
                      ...
                      <div className="col-span-4 space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold text-slate-100 truncate">{feature.name || 'Unnamed Feature'}</span>
                      ...
                      <div className="col-span-2 flex items-center justify-center gap-1.5 min-w-0">
                        {feature.devAvatar ? (
                          <img src={feature.devAvatar} alt={devName} className="w-5 h-5 rounded-full object-cover shrink-0" />
                        ) : (
                        ...
                        <span className="text-xs text-slate-300 font-medium truncate">{devFirstName}</span>
                      </div>

                      {/* Right: Timeline Bar */}
                      <div className="col-span-6 space-y-1">
                        <div className="flex justify-between text-[10px] font-mono text-slate-400">
                          <span>Target: {feature.deadline || 'N/A'}</span>
